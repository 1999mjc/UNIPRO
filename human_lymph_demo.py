...
OUTDIR = Path(__file__).resolve().parent / "fused_edge_graph_demo"
...
def save_edge_table(name, data):
    ...
    edge_df.to_csv(OUTDIR / f"{name}_fused_edges.csv", index=False)
...
def plot_metric_boxplot(metrics_df, out_path):
    ...
    plt.title("Evaluation Metrics for Slice B (A -> B Transfer, fused edge weights)", fontsize=15)
...
def main():
    ...
    best_model_path = WEIGHTS_DIR / "best_model_A_to_B.pt"
